...
local atms = {
  ...
  {name="ATM", id=277, x=147.598, y=-1035.690, z=29.343},
  {name="ATM", id=277, x=145.965, y=-1035.156, z=29.345},
  {name="ATM", id=277, x=-1391.003, y=-590.300, z=30.320},

}

-- Banks
local banks = {
  {name="Bank", id=108, x=-262.436, y=794.169, z=118.602},
}
...
--[[if enableBankingGui then
  Citizen.CreateThread(function()
    ...
    while true do
      Citizen.Wait(0)
      if IsControlJustReleased(0, tab_key)  then -- IF INPUT_PICKUP Is pressed
      ...
      --Draw the markers for banks near by.
   local markerBanks = GetBanksInRange(50.0)   
      for _, bank in pairs(markerBanks) do
      ...
      if(IsNearBank() or IsNearATM()) then
        ...
        if nearestBank ~= nil then
          ...
          AddTextComponentString("Press ~INPUT_VEH_HORN~ to open the ~g~bank~s~ menu")
          DisplayHelpTextFromStringLabel(0, 0, 0, -1)
        end
        
        atBank = true

        if IsControlJustReleased(0, keys["RIGHTBRACKET"])  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You cannot use the bank in a vehicle!");
          else
            if bankOpen then
...
function GetNearestBank()
  ...
  local bankDist = 9999999999
  local nearestBank = nil

  for _, bank in pairs(banks) do
    local distance = GetDistanceBetweenCoords(bank.x, bank.y, bank.z,  plyCoords["x"], plyCoords["y"], plyCoords["z"], true)
...
-- Check if player is near a bank
function GetBanksInRange(range)
  local ply = GetPlayerPed(-1)
  local plyCoords = GetEntityCoords(ply, 0)
  local nearestBanks = {}
  local bankCount = 1
